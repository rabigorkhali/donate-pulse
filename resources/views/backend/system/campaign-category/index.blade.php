        ...
        <div class="card mb-4">
            ...
            <div class="card">
                <div class="card-datatable table-responsive">
                    <form method="get" action="{{ route('campaign-categories.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('campaign-categories.create') }}">
                            ...
                            <table class="datatables-campaign-categories table dataTable no-footer dtr-column" id="DataTables_Table_0">
                                ...
                                @endif
                                @foreach ($thisDatas as $index => $category)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $category->title }}</td>
                                        <td>{{ $category->slug }}</td>
                                        <td>
                                            <span class="badge @if ($category->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($category->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                            ...
                                            @if(hasPermission('/'.strtolower($title).'/*','put') || hasPermission('/'.strtolower($title).'/*','delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                            <a class="dropdown-item" href="{{ route('campaign-categories.edit', $category->id) }}">
                                                                <i class="ti ti-pencil me-1"></i>{{ __('Edit') }}
                                                            </a>
                                                        @endif
                                                        @if(hasPermission('/'.strtolower($title).'/*','delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('campaign-categories.destroy', $category->id) }}" data-bs-target="#deleteModal">
