                ...
                <div class="card-datatable table-responsive">
                    <form method="get" action="{{ route('partners.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('partners.create') }}">
                            ...
                            <table class="datatables-teams table dataTable no-footer dtr-column" id="DataTables_Table_0">
                                ...
                                @endif
                                @foreach ($thisDatas as $index => $partner)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $partner->name }}</td>
                                        ...
                                        <td>
                                            @if($partner->image)
                                                <img src="{{ asset($partner->image) }}" width="50" alt="Image" class="img-fluid">
                                            @else
                                                N/A
                                            @endif
                                        </td>
                                        <td>
                                            <span class="badge @if ($partner->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($partner->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                            ...
                                            @if(hasPermission('/'.strtolower($title).'/*','put') || hasPermission('/'.strtolower($title).'/*','delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                            <a class="dropdown-item" href="{{ route('partners.edit', $partner->id) }}">
                                                                <i class="ti ti-pencil me-1"></i>{{ __('Edit') }}
                                                            </a>
                                                        @endif
                                                        @if(hasPermission('/'.strtolower($title).'/*','delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('partners.destroy', $partner->id) }}" data-bs-target="#deleteModal">
